/**
 * Delegate interface for UI updates from MicrophoneDetector.
 * @typedef {Object} MicrophoneDetectorDelegate
 * @property {(level: number) => void} [onLevelChanged] - Audio level changed (0-100)
 * @property {(peak: number) => void} [onPeakChanged] - Peak hold changed (0-100)
...
/**
 * MicrophoneDetector - Pure domain logic for audio hit detection.
 * Does not interact with DOM. Uses delegate pattern for behavioral updates.
 * Dependencies are injected to enable testing and inversion of control.
 */
class MicrophoneDetector {
  /**
   * @param {Object} storageManager - Storage instance for persisting settings
   * @param {MicrophoneDetectorDelegate} delegate - Delegate for behavioral updates
