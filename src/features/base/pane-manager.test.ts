...
Deno.test(
  "PaneManager.getInitialPane: onboarded + calibrated → 'plan-play'",
  () => {
    const pane = PaneManager.getInitialPane({
...
Deno.test(
  "PaneManager.getInitialPane: onboarded but no calibration → 'onboarding'",
  () => {
    const pane = PaneManager.getInitialPane({
      hasCompletedOnboarding: true,
      hasCalibration: false,
...
Deno.test(
  "PaneManager constructor does NOT fire pane change callbacks",
  async () => {
...
Deno.test(
  "PaneManager.initialize() fires onPaneChange callback with current hash pane",
  async () => {
...
Deno.test(
  "PaneManager.initialize() defaults to 'onboarding' when hash is empty",
  async () => {
...
Deno.test(
  "PaneManager.initialize() fires all registered onPaneChange callbacks",
  async () => {
...
Deno.test(
  "PaneManager.getCurrentPane() returns null before initialize()",
  async () => {
    await resetHash("onboarding");
    const pm = new PaneManager();
    assertEquals(pm.getCurrentPane(), null);
...
Deno.test(
  "PaneManager.getCurrentPane() returns pane name after initialize()",
  async () => {
...
Deno.test(
  "PaneManager.updateVisibility() calls onShow on incoming pane elements",
  async () => {
    await resetHash("onboarding");
...
Deno.test(
  "PaneManager.updateVisibility() calls onHide on outgoing pane elements",
  () => {
    createPaneDom(["onboarding", "plan-play"]);
...
Deno.test(
  "PaneManager.updateVisibility() hides all panes except target",
  async () => {
    await resetHash("onboarding");
    ...
    const onboardingEl = document.getElementById(
      "pane-onboarding",
    ) as HTMLElement;
    const planPlayEl = document.getElementById("pane-plan-play") as HTMLElement;
    const calibrationEl = document.getElementById(
      "pane-calibration",
    ) as HTMLElement;
...
Deno.test(
  "PaneManager.updateVisibility() marks nav button active for target pane",
  async () => {
    await resetHash("onboarding");
    createPaneDom(["onboarding", "plan-play"]);
...
Deno.test(
  "PaneManager.updateVisibility() skips onHide if no previous pane",
  async () => {
    await resetHash("onboarding");
    createPaneDom(["onboarding", "plan-play"]);
