...
async function createComponent() {
  ...
  const viewport = component.querySelector("[data-timeline-viewport]") as HTMLElement;
...
Deno.test("TimelineVisualization: should render groups and expectations from plan", async () => {
  const component = await createComponent();
  ...
  assertEquals(component.querySelectorAll(".timeline-expectation-filled").length, 4);
});

Deno.test("TimelineVisualization: should position detections using display start beat", async () => {
  const component = await createComponent();
  ...
  const detection = component.querySelector(".timeline-detection") as HTMLElement;

  assertEquals(appended, true);
  assertEquals(detection.style.left, "300px");
});

Deno.test("TimelineVisualization: centerAt should scroll track and track last beat", async () => {
  const component = await createComponent();
